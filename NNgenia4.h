...
public:
	...
	Driver_joint m_driver;


	NNgenia4(const Options &options):m_options(options) {
		B  = "B";
		I = "I";
		HB = "HB";
		HI = "HI";
	...
	BestPerformance trainAndTest(Tool& tool,
		...
		m_driver._modelparams.labelAlpha_relation.set_fixed_flag(false);
		m_driver._modelparams.labelAlpha_relation.from_string(N);
		m_driver._modelparams.labelAlpha_relation.from_string(Y);
		m_driver._modelparams.labelAlpha_relation.set_fixed_flag(true);
		cout << "total relation label size "<< m_driver._modelparams.labelAlpha_relation.size() << endl;
		...
		for (int iter = 0; iter < m_options.maxIter; ++iter) {
		    ...
		    for (int updateIter = 0; updateIter < batchBlock; updateIter++) {
				...
				if(subExamples.size()>0) {
					dtype cost = m_driver.train_entity(subExamples, curUpdateIter);
					//m_driver.checkgrad_entity(subExamples, curUpdateIter + 1);
				}

				if(subRelationExamples.size()>0) {
					m_driver.train_relation(subRelationExamples, curUpdateIter);
					// m_driver.checkgrad_relation(subExamples, curUpdateIter + 1);
				}
...
/*				eval.overall_label_count += m_driver._eval.overall_label_count;
				...
				if(subExamples.size()>0 || subRelationExamples.size()>0) {
		...
		for(int docIdx=0;docIdx<documents.size();docIdx++) {
			...
			for(int sentIdx=0;sentIdx<doc.sentences.size();sentIdx++) {
				...
				for(int tokenIdx=0;tokenIdx<sent.info.tokens.size();tokenIdx++) {
					fox::Token& token = sent.info.tokens[tokenIdx];

					string labelName = O;
					getSchameLabel(sent.info, tokenIdx, entities, labelName);
					outputs.push_back(labelName);
				}

				vector<Entity> spanEntities;
				decode(sent.info, outputs, spanEntities);
				...
				for(int latterIdx=0;latterIdx<spanEntities.size();latterIdx++) {
					...
					for(int formerIdx=0;formerIdx<latterIdx;formerIdx++) {
						...
						Example eg;
						generateOneRelExample(eg, sent.info, formerSpan, latterSpan, entityContainFormerSpan,
	...
	BestPerformance evaluateOnDev(Tool& tool, vector<Document>& documents) {
		...
		for(int docIdx=0;docIdx<documents.size();docIdx++) {
			...
			for(int sentIdx=0;sentIdx<doc.sentences.size();sentIdx++) {
				...
				vector<int> labelIdx;
				m_driver.predict_entity(eg.m_features, labelIdx);
				...
				for(int latterIdx=0;latterIdx<spanEntities.size();latterIdx++) {
					...
					for(int formerIdx=0;formerIdx<latterIdx;formerIdx++) {
						...
						Example egRel;
						generateOneRelExample(egRel, sent.info, formerSpan, latterSpan, entityContainFormerSpan,
								entityContainLatterSpan, true);

						int result = -1;
						m_driver.predict_relation(egRel, result);
