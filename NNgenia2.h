...
public:
	...
	NNgenia2(const Options &options):m_options(options) {
		B  = "B";
		I = "I";
		HB = "HB";
		HI = "HI";
...
/*								for(int i=0;i<eg.m_labels.size();i++) {
									int idx = -1;
									for(int j=0;j<eg.m_labels[i].size();j++) {
										if(eg.m_labels[i][j] == 1)
											idx = j;
									}
									cout<<NERlabelID2labelName(idx)<<" ";
								}
								cout<<endl;*/
